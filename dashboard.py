...
from analysis import main as run_analysis_pipeline
from load_data import main as run_load_pipeline
...
def _database_is_ready() -> bool:
    """Return whether the generated database contains every app object."""
    ...
    return REQUIRED_DATABASE_OBJECTS <= available_objects


def require_database() -> bool:
    """Build the generated database on first run, including on Streamlit Cloud."""
    if _database_is_ready():
        return True

    try:
        with st.spinner("Preparing the analysis database for this app..."):
            run_load_pipeline()
            run_analysis_pipeline()
    except Exception as exc:
        st.error(
            "The app could not prepare its SQLite database. "
            f"Check the source data and deployment logs. Details: {exc}"
        )
        return False

    if not _database_is_ready():
        st.error(
            "The analysis pipeline finished without creating all required database "
            "tables. Check the deployment logs."
        )
        return False
